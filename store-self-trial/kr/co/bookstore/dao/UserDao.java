...
/**
 * STORE_USERS 테이블에 대한 CRUD 기능을 제공하는 클래스
 * @author doyoung
 *
 */
public class UserDao {
	...
	/**
	 * 이메일을 제공받아서 이메일이 일치하는 사용자정보를 반환한다.
	 * @param email
	 * @return 사용자정보, 이메일이 일치하는 사용자정보가 없으면 null을 반환한다.
	 * @throws SQLException 데이터베이스 액세스 작업 중 오류가 발생하면 이 예외를 던진다.
	 */
	public User getUserByEmail(String email) throws SQLException {
		String sql = "select *" // 실제로는 속성명 다 적어야 한다.
					+ "from store_users "
					+ "where user_email = ? ";
	...
	/**
	 * 사용자번호를 제공받아서 사용자번호가 일치하는 사용자정보를 반환한다.
	 * @param userNo
	 * @return 사용자정보, 이메일이 일치하는 사용자정보가 없으면 null을 반환한다.
	 * @throws SQLException 데이터베이스 액세스 작업 중 오류가 발생하면 이 예외를 던진다.
	 */
	...
	/**
	 * 
	 * @param rs ResultSet 객체를 제공받아서 정보를 User객체에 반영해 반환한다.
	 * @return 사용자정보
	 * @throws SQLException 데이터베이스 액세스 작업 중 오류가 발생하면 이 예외를 던진다.
	 */
	...
	public void updateUser(User user) throws SQLException {
		...
		pstmt.setString(5, user.getDeleted());
